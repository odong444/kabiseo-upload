...
def campaigns():
    ...
    for c in campaign_list:
        ...
        # 모집전 판단 (시작일 이전)
        if start_date_str and c.get("상태") in ("모집중", "진행중", ""):
            try:
                if _datetime.now(_KST).date() < _datetime.strptime(start_date_str, "%Y-%m-%d").date():
                    c["_not_started"] = True
            except Exception:
                pass
...
def _do_campaign_edit_post(campaign_id):
    editable_fields = [
        ...
        "공개여부", "캠페인가이드", "추가안내사항", "메모", "홍보메시지",
        "홍보활성", "홍보카테고리", "홍보시작시간", "홍보종료시간",
        "AI구매검수지침", "AI리뷰검수지침",
    ]
...
@admin_bp.route("/api/campaigns/<campaign_id>/promo-message", methods=["POST"])
@admin_required
def api_campaign_promo_message(campaign_id):
    if not models.db_manager:
        return jsonify({"ok": False, "message": "시스템 초기화 중"})
    try:
        msg = request.json.get("message", "")
        models.db_manager.update_campaign(campaign_id, {"홍보메시지": msg})
        return jsonify({"ok": True})
    except Exception as e:
        logger.error(f"홍보메시지 저장 에러: {e}")
        return jsonify({"ok": False, "message": str(e)})
