...
- ID를 모르면 list_agencies 또는 list_clients로 먼저 조회하여 ID 확인 후 설정
- 예: 사용자가 "이 캠페인 대행사를 ○○로 설정해줘" → list_agencies(query="○○")로 ID 찾기 → update_campaign(changes={"대행사ID": ID})
    ...
    },
    {
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "changes": {
                    "type": "object",
                    "description": "변경할 필드와 값. 키는 한국어 컬럼명: 상품명, 캠페인명, 업체명, 플랫폼, 총수량, 결제금액, 일수량, 진행일수, 리뷰비, 상태, 공개여부, 대행사ID, 클라이언트ID, 옵션, 상품링크, 키워드, 유입방식, 구매가능시간, 중복허용, 캠페인가이드, 추가안내사항, 리뷰기한일수, 시작일, 일정 등"
                }
            },
