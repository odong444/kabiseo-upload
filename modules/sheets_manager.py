...
캠페인관리: 캠페인ID, 등록일, 상태, 업체명, 상품명, 상품링크, 옵션, 키워드,
  ...
  결제금액, 리뷰가이드, 중복허용, 리뷰비,
  상품번호, 캠페인유형, 플랫폼, 키워드위치, 옵션지정방식, 옵션목록,
  체류시간, 상품찜필수, 알림받기필수, 광고클릭금지, 결제방법,
  블라인드계정금지, 재구매확인, 구매가능시간, 한달중복허용,
  배송메모필수, 배송메모내용, 배송메모안내링크,
  리뷰타입, 리뷰가이드내용, 리뷰이미지폴더, 추가안내사항,
  신청마감일, 공개여부, 선정여부, 상품이미지, 상품금액, 리워드
"""
...
class SheetsManager:
    ...
    def count_today_all_campaigns(self) -> dict:
        ...
        date_col = self._find_col(headers, "날짜")
        status_col = self._find_col(headers, "상태")
        if cid_col < 0 or date_col < 0:
            return {}
        counts = {}
        for row in ws.get_all_values()[1:]:
            if len(row) <= max(cid_col, date_col):
                continue
            if row[date_col] != today:
                continue
            if status_col >= 0 and len(row) > status_col:
                if row[status_col] in ("타임아웃취소", "취소"):
                    continue
